...
public class EmployeeDAO implements DAO<Employee, String>{
    ...
    // lay ra ca nhan vien chua cham cong trong ngay hom nay
    public List<Employee> selectEmployee(String name) {
        StringBuilder sql = new StringBuilder()
                .append("SELECT * FROM nhanvien")
                .append(" WHERE tennhanvien LIKE ? AND trangthai = 1")
                .append(" AND nhanvien.manhanvien NOT IN (SELECT manhanvien FROM luong WHERE TO_CHAR(ngaydilam, 'yyyy/MM/dd') = TO_CHAR(now(), 'yyyy/MM/dd'))");
        
        return this.selectBySQL(sql.toString(), "%"+name+"%");
    }
}
